...
python <this-skill>/scripts/autopilot_state.py config-set --repo <repo> --smoke-commands 'python -m compileall -q .'
...
`deny_paths`, `secret_patterns`, `track_state`) are deliberately
not settable at runtime — changing those mid-run requires editing the file by
hand, which surfaces the config-drift warning on the next `check`. The
verification commands are settable on purpose (`--check-commands` /
`--smoke-commands`) so a run whose verification is wrong can be corrected
without hand-editing; treat them as configuration, not as a guard-rail.
...
Boolean, default `true`. When true, `commit` scans the **added lines** of the staged diff for secret-like content before committing and refuses on a match. Built-in patterns cover AWS access keys (`AKIA...`), private-key blocks, GitHub tokens (`ghp_`/`gho_`/`ghs_`/`ghu_`/`ghr_` and `github_pat_`), Slack tokens (`xox...`), Google API keys (`AIza...`), OpenAI-style `sk-...` keys (including `sk-proj-`/`sk-ant-`), JWTs (`eyJ...`), and `api_key = ...` assignments. The scan is line-based plaintext matching: secrets split across lines or wrapped in Base64 stay outside its reach. If `git diff` fails during the scan, the helper **fails closed** (treats the scan as dirty) instead of allowing the commit. Bypass a false positive with `commit --allow-secrets`, or disable entirely by setting this to `false` (`check` then surfaces a standing warning). Run `secret-scan` at any time to check the staged diff without committing. Use `secret_patterns` to add repository-specific regexes.
...
Integer, default `2`. After this many candidates of the same `type` have been completed, `backlog-rank` downweights further same-type pending candidates by logarithmic decay with a hard floor (`max(0.35, 1.0 - 0.12 * log2(1 + max(0, completed - threshold)))` in the default `ranking_mode: expected`; `0.85` power decay in `classic`). Saturation dents the diversity signal but must never veto a genuinely valuable candidate of the dominant type. This stops the loop from grinding out the same low-hanging-fruit category forever. Set to a large number to disable.
...
Non-negative integer, default `null` (unlimited). Caps how many Deep Expansion waves a run may record. It exists because `max_tokens` cannot see subagent spend: that estimate is derived from the run's diff (`500 + 12 x changed lines + ...`), and a wave's tokens are spent inside subagents, never in a diff. Without a wave cap, an `expand_after_goals` run or a thin-backlog run can spend arbitrarily more than its configured budget while every `check` reports the budget as healthy. Counting is monotonic (`state.expansion_wave_seq`), independent of the 20-entry rolling window in `state.expansion_waves`. `check` reports `expansion_budget` (`waves_used`/`max_waves`/`waves_left`) and warns once the cap is reached; `expansion-record` refuses to record past it and points at `config-set --max-expansion-waves N`. Unlike the other guard-rail fields this one is settable at runtime on purpose: hitting a cap mid-run needs an escape hatch that is not "stop the run". Init flag: `--max-expansion-waves N`.
...
Array of strings, default `[]`. Cheap checks to run between verification rounds (for example `["python -m compileall -q ."]`). They exist because the loop verifies fully only every `verify_every_rounds` rounds, and an improvised smoke check differs per round and leaves no trace in the config — declare one once and every round runs the same thing. A smoke command must stay syntax/type-level (`python -m compileall`, `node --check`, `cargo check`, `go vet`): anything that runs the test suite is the full verification wearing a cheaper name. A passing smoke run is never proof of correctness and never excuses skipping the full set on a verify/commit round. `detect-verify` prints `smoke_recommended` for the repo's technology; record it with `config-set --smoke-commands '<cmd>'` or `--clear-smoke-commands`. Init flag: `--smoke-commands CMD` (repeatable).
...
- `init` — create config + state (+ optional feature branch). Flags cover every config field: `--goal`, `--goals-from-prompt`, `--max-rounds`, `--max-minutes`, `--deadline`, `--max-tokens`, `--max-round-scope`, `--branch-mode`, `--allow-uncommitted-changes`, `--track-state`, `--check-commands`, `--smoke-commands`, `--push`, `--commit-message-prefix`, `--retries-per-round`, `--candidates-per-round`, `--commit-every-rounds`, `--verify-every-rounds`, `--checkpoint-every`, `--expand-after-goals`, `--review-threshold`, `--scan-secrets`/`--no-scan-secrets`, `--secret-pattern`, `--type-saturation-threshold`, `--ranking-mode`, `--min-candidate-value`, `--max-same-type-per-round`, `--min-pending-candidates`, `--max-predicted-per-round`, `--max-expansion-per-round`, `--max-expansion-waves`, `--max-blocked-in-a-row`, `--allow-path`, `--deny-path`, `--report-lang`, `--force`.
- `read`, `check`, `diagnose` — inspect state, stop conditions, and repository/git health. `check --brief` returns loop-driving fields only: `continue`/`stop_reason`/`warnings`/`goals_met`/`phase`/`backlog` (`pending`/`ready`/`min_pending_candidates`/`needs_expansion`)/`action_hint` (`work`|`mine`|`expand`|`stop`)/`next_verify_round`/`next_commit_round`/`next_checkpoint_round`/`blocked_streak`/`wave_no`/`lenses_used`/`lenses_unused`/`analysis`/`mining`/`budget` (`remaining_minutes`/`deadline_remaining_minutes`) (saves tokens in the loop). In the expand phase an `expansion` object with open seeds and type context is appended (see Direction Seeds above).
...
- `detect-verify` — scan repo entry points and recommend `check_commands` (including `gitleaks`/`detect-secrets` when installed); `--apply` writes them into the config. It also reports `smoke_recommended` (syntax-level checks per detected technology) as a suggestion only — record one with `config-set --smoke-commands`, never by promoting it into `check_commands`.
- `round-prep` — the loop's round start in one call: the `check` payload plus `candidates` (brief form, `candidates_per_round + 1`, with `recommended` ids and `withheld`), the cached analysis when fresh, the standing `directives`, `open_round` and `next_round_number` (from `round_seq`, the sequence `begin-round` uses). Replaces check + analysis-load + backlog-rank + directive-list for a round-opening agent; `check --brief` alone is still the cheapest stop-condition probe.
- `mine` — deterministic repo mining into backlog candidates via seven scanners (`markers`/`swallowed`/`syntax`/`test-gap`/`hotspot`/`dead-export`/`docs-drift`); `--apply` writes deduped, evidence-backed candidates (`from_mine`, evidence `kind | file:line`), `--kind K` filters one scanner, `--limit N` caps per-kind results (explicit `0` returns nothing). Every non-dry-run invocation appends to `state.mining_runs` (bounded at 20; records `findings`/`applied`/`new`/`apply`); `check` surfaces the `mining` payload (`runs`/`last_findings`/`last_applied`/`exhausted`) and sets `action_hint: "mine"` when the backlog is thin and mining is stale or never ran. Exhaustion means two consecutive runs with zero **new** findings — a repo with resident findings is never exhausted, and never-mined is not exhausted. Read-only probes (no `--apply`) that found nothing still count as zero-new evidence, but probes that found something do not: without `--apply` nothing enters the backlog, so their new-count would misread as churn forever.
